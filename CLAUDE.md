...
### Правила оформления математики (ENF)

Канон — репозиторий пособия `C:/Math_for_DS_&_RL` (`docs/02_Color_System.md`,
`docs/03_Math_Notation.md`). Проверка: **`npm run math:audit`** — обязательна после правки формул.

- **Роль, а не буква.** Тон закреплён за ролью сущности в конкретной формуле:
  `variable` — объект, над которым действуем (`s`, `x`); `function` — преобразование
  (`f`, `\sigma`, `D`); `parameter` — то, что настраивается (`\gamma`, `\theta`, `\alpha`);
  `operator` — агрегат (`V`, `Q`, `G`, `\nabla`); `target` — цель (`r`, `L`, `J`).
  Одна буква в разных формулах может иметь разные роли — это не ошибка.
- **Лимит тонов.** Страницы курса — learning mode: не более 4 тонов в формуле,
  окрашено ~40 % символов. Длинное уравнение Беллмана в лимит не влезает, поэтому
  функциональный слой (`\pi`, `P`, `a`) в нём остаётся нейтральным.
- **Что не окрашивается никогда:** скобки, знаки операций, запятые и **индексы**.
  Индекс выносится за макрос: `\enfTgt{r}_{t+1}`, а не `\enfTgt{r_{t+1}}`.
- **Домовые соглашения пособия:** `\mathbb{E}` всегда нейтральна; отдача `G` — `operator`,
  а не `target`; счётчики (`i`, `n`, `t`, `k`, `N`, `B`) тона не получают.
- **Акцент — внутрь роли:** `\enfPar{\bar{\theta}}`. Обратный порядок
  (`\bar\enfPar{\theta}`) роняет разбор: акцент забирает сам макрос.
- **Нотация:** `\operatorname{Var}` (не `\text{Var}`), `\mathbb{E}` (не `E`),
  `\mathrm{d}x` (не `dx`), `\mid` (не `|`) в условной вероятности.
- Окрашенная сущность должна быть объяснена словами рядом — цвет указывает
  на объяснение, а не заменяет его.

Локальные обёртки KaTeX (`<F>`, `<IM>`, `<Tex>`) обязаны рендерить с `KATEX_OPTIONS`
из `src/lib/katex-options.ts` — иначе макросы ролей молча не сработают и формула
останется одноцветной.
...
**`src/config/lessonAccess.ts`** — отдельный от монетизации флаг `SEQUENTIAL_LESSON_LOCK`
(сейчас `false`). Он управляет «Картой обучения» на главной (`LearningPathSection` +
`useLearningProgress`): при `false` уроки открыты в любом порядке, при `true` доступен только
следующий после пройденного, остальные — с замком. Статусы урока: `completed` / `current`
(следующий по порядку, подсвечен) / `available` (открыт, замок выключен) / `locked`.
