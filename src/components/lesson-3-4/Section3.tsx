...
const Section3 = () => (
  <>
    ...
    <ProseP>
      ...
      <Math display={false}>{String.raw`\psi`}</Math>:
    </ProseP>
    <Math display>
      {String.raw`\operatorname{IRL}_\psi(\pi_E)=\arg\max_{c\in\mathbb{R}^{\mathcal{S}\times\mathcal{A}}}\ -\psi(c)+\Big(\min_{\pi\in\Pi}-H(\pi)+\mathbb{E}_\pi[c(s,a)]\Big)-\mathbb{E}_{\pi_E}[c(s,a)],`}
    </Math>
    <ProseP>
      где <Math display={false}>{String.raw`H(\pi)=\mathbb{E}_\pi[-\log\pi(a\mid s)]`}</Math> —{" "}
      <Math display={false}>{String.raw`\gamma`}</Math>-дисконтированная causal entropy, а{" "}
      <Math display={false}>{String.raw`\operatorname{RL}(c)=\arg\min_\pi -H(\pi)+\mathbb{E}_\pi[c(s,a)]`}</Math>
      .
    </ProseP>

    <h3 className={H3_CLASS}>Occupancy measure — мост к биекции</h3>
    <Math display>
      {String.raw`\rho_\pi(s,a)=\pi(a\mid s)\sum_{t=0}^{\infty}\gamma^t P(s_t=s\mid\pi),`}
    </Math>
    <ProseP>
      что позволяет писать{" "}
      <Math display={false}>{String.raw`\mathbb{E}_\pi[c(s,a)]=\sum_{s,a}\rho_\pi(s,a)\,c(s,a)`}</Math>
      .
    </ProseP>

    <Callout title="Лемма 3.1 (биекция Π ↔ D)" color="purple">
      Если <Math display={false}>{String.raw`\rho\in\mathcal{D}`}</Math> (множество допустимых
      occupancy measures), то <Math display={false}>{String.raw`\rho`}</Math> — occupancy measure{" "}
      <strong>единственной</strong> политики{" "}
      <Math display={false}>{String.raw`\pi_\rho(a\mid s)=\rho(s,a)/\sum_{a'}\rho(s,a')`}</Math>. Между{" "}
      <Math display={false}>{String.raw`\Pi`}</Math> и{" "}
      <Math display={false}>{String.raw`\mathcal{D}`}</Math> существует биекция — поэтому работать с
      occupancy так же законно, как с политикой.
    </Callout>

    <Callout title="Proposition 3.1 (Ho & Ermon, Eq. 4)" color="purple">
      <Math display>
        {String.raw`\operatorname{RL}\circ\operatorname{IRL}_\psi(\pi_E)=\arg\min_{\pi\in\Pi}\ -H(\pi)+\psi^*\big(\rho_\pi-\rho_{\pi_E}\big),`}
      </Math>
      где <Math display={false}>{String.raw`\psi^*`}</Math> — convex conjugate к{" "}
      <Math display={false}>{String.raw`\psi`}</Math>. Смысл: RL после IRL{" "}
      <strong>неявно ищет политику, чья occupancy measure близка к экспертной</strong>; мерой близости
      служит <Math display={false}>{String.raw`\psi^*`}</Math>.
    </Callout>
    ...
    <Math display>
      {String.raw`\psi_{GA}(c)=\begin{cases}\mathbb{E}_{\pi_E}[g(c(s,a))] & c<0\\+\infty & \text{иначе}\end{cases},\qquad g(x)=\begin{cases}-x-\log(1-e^{x}) & x<0\\+\infty & \text{иначе.}\end{cases}`}
    </Math>
    <ProseP>Его сопряжённая (Eq. 14) есть в точности задача бинарной классификации:</ProseP>
    <Math display>
      {String.raw`\psi_{GA}^*(\rho_\pi-\rho_{\pi_E})=\sup_{D\in(0,1)^{\mathcal{S}\times\mathcal{A}}}\ \mathbb{E}_\pi[\log D(s,a)]+\mathbb{E}_{\pi_E}[\log(1-D(s,a))].`}
    </Math>
    <ProseP>Подставляя в Proposition 3.1, получаем минимакс GAIL:</ProseP>
    <Math display>
      {String.raw`\boxed{\ \min_{\pi}\ \max_{D\in(0,1)^{\mathcal{S}\times\mathcal{A}}}\ \mathbb{E}_\pi[\log D(s,a)]+\mathbb{E}_{\pi_E}[\log(1-D(s,a))]-\lambda H(\pi)\ }`}
    </Math>
    <ProseP>
      ...
      <Math display={false}>{String.raw`D^*(s,a)=\rho_{\pi_E}/(\rho_{\pi_E}+\rho_\pi)`}</Math> потеря с
      точностью до константы и масштаба равна{" "}
      <strong>JS-дивергенции</strong>{" "}
      <Math display={false}>{String.raw`D_{JS}(\bar\rho_\pi,\bar\rho_{\pi_E})`}</Math> между
      нормированными occupancy measures — ровно как в стандартном GAN.
    </ProseP>

    <h3 className={H3_CLASS}>Награда для политики</h3>
    <ProseP>
      Из <Math display={false}>{String.raw`\psi_{GA}`}</Math> следует, что шаг RL минимизирует
      стоимость <Math display={false}>{String.raw`c(s,a)=\log D(s,a)`}</Math>; эквивалентно политику
      обучают <strong>максимизировать</strong>
    </ProseP>
    <Math display>{String.raw`r(s,a)=-\log\big(1-D(s,a)\big),`}</Math>
    <ProseP>
      т.е. чем больше дискриминатор «верит», что пара экспертная, тем выше награда. (Возможны
      варианты <Math display={false}>{String.raw`r=\log D - \log(1-D)`}</Math>; в ML-Agents реализован
...
for i = 0, 1, 2, ... do
    ...
    <ProseP>
      ...
      <Math display={false}>{String.raw`D_w`}</Math> — по 2 скрытых слоя (
      <Math display={false}>{String.raw`\tanh`}</Math>); для снижения дисперсии — value-сеть и GAE (
      <Math display={false}>{String.raw`\gamma=0.995`}</Math>,{" "}
      <Math display={false}>{String.raw`\lambda=0.97`}</Math>). TRPO-шаг нужен, чтобы политика не
    ...
    <KeyPoints
      items={[
        ...
        <>
          Награда политике:{" "}
          <Math display={false}>{String.raw`r=-\log(1-D(s,a))`}</Math> — чем больше{" "}
